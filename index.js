...
const requestPermissionsAndroid = (permissionMessage) => {
  ...
  return PermissionsAndroid.check(requiredPermission).then((gotPermission) =>
    gotPermission
      ? true
      : PermissionsAndroid.request(requiredPermission, permissionMessage).then(
...
class CallDetectorManager {
  ...
  constructor(
    ...
    if (Platform.OS === "ios") {
      ...
      this.subscription = new NativeEventEmitter(DeviceEventManagerModule);
      this.subscription.addListener("PhoneCallStateUpdate", (event) => {
        const data = event.split("|");
        this.callback(data[0], data[1]);
      });
    }
  }
